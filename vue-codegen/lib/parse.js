...
let getPathToMethodName2 = function(opts, m, path) {
    if (path === '/' || path === '') {
        return m
    }
    ...
    let result = _.camelCase(segments.join('-'))
    return m.toLowerCase() + result.substring(1)
}
    ...
    _.forEach(swagger.paths, function(api, path) {
        ...
        _.forEach(api, function(op, m) {
            ...
            let method = {
                ...
                methodNameNew: getPathToMethodName(opts, m, path),
                method: m.toUpperCase(),
                isGET: m.toUpperCase() === 'GET',
                isPOST: m.toUpperCase() === 'POST',
                summary: op.description || op.summary,
                tags: op.tags[0].substr(0, op.tags[0].lastIndexOf('-')),
